...
int main(int argc,char **argv)
{
  ...
  N = n*n;
  PetscCall(PetscPrintf(PETSC_COMM_WORLD,"\n2-D SCALED (*wdh*) Laplacian Eigenproblem (matrix-free version), N=%" PetscInt_FMT " (%" PetscInt_FMT "x%" PetscInt_FMT " grid)\n\n",N,n,n));
...
// *wdh*  PetscCall(MatShellSetOperation(A,MATOP_MULT_TRANSPOSE,(void(*)(void))MatMult_Laplacian2D));
// *wdh*   PetscCall(MatShellSetOperation(A,MATOP_GET_DIAGONAL,(void(*)(void))MatGetDiagonal_Laplacian2D));
  ...
  */
  ...
  // Monitor: 
  // use -eps -eps_monitor_all
  PetscViewer viewer;
  PetscViewerASCIIOpen(PETSC_COMM_WORLD,"ex3MonitorOutput.m",&viewer);
  ...
  // PetscBool trackall=true;
  // PetscCall(EPSMonitorSetFromOptions(eps,"-epsMonitorAll","all_approximations",NULL,trackall));
  // --- END --- *wdh*
