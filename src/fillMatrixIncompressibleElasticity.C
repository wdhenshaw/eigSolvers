// This file automatically generated from fillMatrixIncompressibleElasticity.bC with bpp.
                                                                        ...
                                                                        int saveMatlab, int useWideStencils )
{
  ...
  //      = traction     : include boundary
    ...
    {
              ig0 = (i1)-n1a + nd1a*( (i2)-n2a + nd2a*(psim) );
              assert( ig0>=0 && ig0<N );
     ...
     // --- loop over stencil ---
         ...
         // coeff = iCoeff(iw1,iw2); // *************************** TEMP
                  if( coeff != 0. )
                  {
                            ig = (i1+iw1)-n1a + nd1a*( (i2+iw2)-n2a + nd2a*(psim) );
                            assert( ig>=0 && ig<N );
                        ierr = MatSetValue(A,ig0,ig, coeff,INSERT_VALUES);CHKERRQ(ierr); 

                            if( printEntries )
                            {
                                printF(" ig0=%4d ig=%4d value=%10.3f (interiorPsi)\n",ig0,ig,coeff);
                            }
                    }
              }
          }
    ...
    //    =traction     : exclude boundary
        ...
        {
                ig0 = (i1)-n1a + nd1a*( (i2)-n2a + nd2a*(pm) );
                assert( ig0>=0 && ig0<N );
      // printF("Interior: ig0=%d, i1=%d, i2=%d\n",ig0,i1,i2);
      // --- FILL Delta for p ----
      // --- loop over stencil ---
            for( int iw2=-hw; iw2<=hw; iw2++ )
            {
                for( int iw1=-hw; iw1<=hw; iw1++ )
                {
                    ...
                    {
                              ig = (i1+iw1)-n1a + nd1a*( (i2+iw2)-n2a + nd2a*(pm) );
                              assert( ig>=0 && ig<N );
                          ierr = MatSetValue(A,ig0,ig, -lapCoeff(iw1,iw2),INSERT_VALUES);CHKERRQ(ierr);   // flip sign *fix me*

                              if( printEntries )
                              {
                                  printF(" ig0=%4d ig=%4d value=%10.3f (interiorP)\n",ig0,ig,-lapCoeff(iw1,iw2));
                              }
                      }
                }
            }
    ...
    ForBoundary(side,axis)
    {
        ...
        // --- Dirichlet BC for psi :
                ierr = MatSetValue(A,ig0,ig0, 1.,INSERT_VALUES);CHKERRQ(ierr);
                    ...
                    if( printEntries )
                    {
                        printF(" ig0=%4d ig=%4d value=%10.3f (psiDisplacementBoundaryDirichlet)\n",ig0,ig,1.);
                    }
                        ...
                        for( int iw1=-hw; iw1<=hw; iw1++ )
                        {
                            ...
                            if( coeff != 0. )
                            {
                                ...
                                ierr = MatSetValue(A,ig0,ig, coeff,INSERT_VALUES);CHKERRQ(ierr);  
                                    if( printEntries )
                                    {
                                        printF(" ig0=%4d ig=%4d value=%10.3f (pTractionBC)\n",ig0,ig,coeff);
                                    }
                            }
                        }
                    } 
                        ...
                        for( int iw1=-hw; iw1<=hw; iw1++ )
                        {
                            ...
                            if( coeff != 0. )
                            {
                                ...
                                ierr = MatSetValue(A,ig0,ig, coeff,INSERT_VALUES);CHKERRQ(ierr);  
                                    if( printEntries )
                                    {
                                        printF(" ig0=%4d ig=%4d value=%10.3f (pTractionBCTractionDisplacementCorner)\n",ig0,ig,coeff);
                                    }
                            }
                        }
                    }           
    ...
    ForBoundary(side,axis)
    {
        ...
        FOR_3D(i1,i2,i3,Ib1,Ib2,Ib3) 
        {
            ...
            int j1 = i1 -is1*ghost, j2 = i2 -is2*ghost,  j3 = i3;  // ghost point
                ...
                ig0 = (j1)-n1a + nd1a*( (j2)-n2a + nd2a*(psim) );
                assert( ig0>=0 && ig0<N );
        ...
        // --- Neumann BC on first ghost arg for psi ---
                ...
                for( int iw=-hw; iw<=hw; iw++ )
                {
                      if( axis==0 )
                      {
                              ig = (i1+iw)-n1a + nd1a*( (i2)-n2a + nd2a*(psim) );
                              assert( ig>=0 && ig<N );
                          coeff = xCoeff(iw);
                      }
                      else
                      {
                              ig = (i1)-n1a + nd1a*( (i2+iw)-n2a + nd2a*(psim) );
                              assert( ig>=0 && ig<N );
                          coeff = yCoeff(iw);
                      }
                      if( coeff !=0. )
                      {
                          ierr = MatSetValue(A,ig0,ig, coeff,INSERT_VALUES);CHKERRQ(ierr);
                              ...
                              if( printEntries )
                              {
                                  printF(" ig0=%4d ig=%4d value=%10.3f (psiDisplacementGhost1Neumann)\n",ig0,ig,coeff);
                              }
                      }
        ...
        //    psi.xx - psi.yy = 0 (except at traction-traction corners)
                        ...
                        for( int iw1=-hw; iw1<=hw; iw1++ )
                        {
                            ...
                            if( coeff != 0. )
                            {
                                  ...
                                  ierr = MatSetValue(A,ig0,ig, coeff,INSERT_VALUES);CHKERRQ(ierr);   // flip sign *fix me*
                                      if( printEntries )
                                      {
                                          printF(" ig0=%4d ig=%4d value=%10.3f (psiTractionGhost1)\n",ig0,ig,coeff);
                                      }
                              }
                        }
                    }  
          ...
          // displacement BC
                    ierr = MatSetValue(A,ig0,ig0, 1.,INSERT_VALUES);CHKERRQ(ierr); 
                        if( printEntries )
                        {
                            printF(" ig0=%4d ig0=%4d value=%10.3f (tractionDisplacementExtendedBoundary1PsiDirichlet)\n",ig0,ig0,1.);
                        }
                }      
        ...
        //  mu*( psi.xxy + 3*psi.yyy) + lambda*psi.y = 0 : y=0,1        
                        ...
                        for( int iw1=-hw2; iw1<=hw2; iw1++ )
                        {
                            ...
                            if( coeff != 0. )
                            {
                                ...
                                ierr = MatSetValue(A,ig0,ig, coeff,INSERT_VALUES);CHKERRQ(ierr); 
                                    ...
                                    if( printEntries )
                                    {
                                        printF(" ig0=%4d ig=%4d value=%10.3f (psiTractionGhost2psixxx)\n",ig0,ig,coeff);
                                    }
                            }
          ...
          // displacement BC
                    ierr = MatSetValue(A,ig0,ig0, 1.,INSERT_VALUES);CHKERRQ(ierr);  
                        if( printEntries )
                        {
                            printF(" ig0=%4d ig0=%4d value=%10.3f (tractionDisplacementExtendedBoundary2PsiDirichlet)\n",ig0,ig0,1.);
                        }
                }
      ...
      // -- extrapolate other ghost 
        ...
        // printF("Ghost: ig0=%d, j1=%d, j2=%d\n",ig0,j1,j2);
                ...
                {
                        ig = (j1+is1*k)-n1a + nd1a*( (j2+is2*k)-n2a + nd2a*(psim) );
                        assert( ig>=0 && ig<N );
                    ierr = MatSetValue(A,ig0,ig,extraplapCoeff[k],INSERT_VALUES);CHKERRQ(ierr); 
                        if( printEntries )
                        {
                            printF(" ig0=%4d ig=%4d value=%10.3f (extrapGhostPsi)\n",ig0,ig,extraplapCoeff[k]);
                        }
                }
            }
        ...
        if( includePressure ) 
        {
            ...
            FOR_3D(i1,i2,i3,Ib1,Ib2,Ib3) 
            {
                ...
                int j1 = i1 -is1*ghost, j2 = i2 -is2*ghost,  j3 = i3;  // ghost point
                    ...
                    ig0 = (j1)-n1a + nd1a*( (j2)-n2a + nd2a*(pm) );
                    assert( ig0>=0 && ig0<N );
          ...
          //   p.y = mu*v.yy => p.y + mu*psi.xyy =0 
                    ...
                    for( int iw=-hw; iw<=hw; iw++ )
                    {
                          if( axis==0 )
                          {
                                  ig = (i1+iw)-n1a + nd1a*( (i2)-n2a + nd2a*(pm) );
                                  assert( ig>=0 && ig<N );
                              coeff  = xCoeff(iw);
                          }
                          else
                          {
                                  ig = (i1)-n1a + nd1a*( (i2+iw)-n2a + nd2a*(pm) );
                                  assert( ig>=0 && ig<N );
                              coeff = yCoeff(iw);
                          }
                          if( coeff !=0. )
                          {
                              ierr = MatSetValue(A,ig0,ig, coeff,INSERT_VALUES);CHKERRQ(ierr); 
                                  ...
                                  if( printEntries )
                                  {
                                      printF(" ig0=%4d ig=%4d value=%10.3f (displacementGhost1p)\n",ig0,ig,coeff);
                                  }
                          }
                    }
          ...
          //   p.x = 0 or p.y = 0
           ...
           //     Delta p = 0 
            ...
            // Try this : just extrapolate
                        for( int k=0; k<=extrapOrder; k++ )
                        {
                                ig = (j1+is1*k)-n1a + nd1a*( (j2+is2*k)-n2a + nd2a*(pm) );
                                assert( ig>=0 && ig<N );
                            ierr = MatSetValue(A,ig0,ig,extraplapCoeff[k],INSERT_VALUES);CHKERRQ(ierr); 

                                if( printEntries )
                                {
                                    printF(" ig0=%4d ig=%4d value=%10.3f (extrapGhostTractionDisplacementCornerP)\n",ig0,ig,extraplapCoeff[k]);
                                }
                        }
                    }
          ...
          // printF("Ghost: ig0=%d, j1=%d, j2=%d\n",ig0,j1,j2);
                    for( int k=0; k<=extrapOrder; k++ )
                    {
                        ...
                        ierr = MatSetValue(A,ig0,ig,extraplapCoeff[k],INSERT_VALUES);CHKERRQ(ierr); 
                            ...
                            if( printEntries )
                            {
                                printF(" ig0=%4d ig=%4d value=%10.3f (extrapGhostp)\n",ig0,ig,extraplapCoeff[k]);
                            }
                    }
                }
            ...
            // printF("Corner Ghost: ig0=%d, j1=%d, j2=%d\n",ig0,j1,j2);
              ...
              // even symmetry corner for psi and p : u(-1,-1) = u(1,1)   **check me for p**
                            ...
                            int j1s = i1+is1*ghost1, j2s= i2+is2*ghost2, j3s=j3;   // symmetric point inside
                                ig = (j1s)-n1a + nd1a*( (j2s)-n2a + nd2a*(n) );
                                assert( ig>=0 && ig<N );
              ...
              // extrapolate corner 
                            ...
                            for( int k=0; k<=extrapOrder; k++ )
                            {
                                ...
                                if( n==0 ) 
                                {
                                        if( printEntries )
                                        {
                                            printF(" ig0=%4d ig=%4d value=%10.3f (extrapCornerGhostPsi)\n",ig0,ig,extraplapCoeff[k]);
                                        }
                                }
                                else
                                {
                                        if( printEntries )
                                        {
                                            printF(" ig0=%4d ig=%4d value=%10.3f (extrapCornerGhostp)\n",ig0,ig,extraplapCoeff[k]);
                                        }
                                }
                            }
                        } 
